...
(async () => {
  ...
  assert.match(server, /const shouldBufferAssistantAudio = \(\) =>[\s\S]*assistantAudioFlushRunning[\s\S]*\);/, 'server should keep buffering assistant audio while a turn or flush is still in flight');
  assert.match(server, /if \(shouldBufferAssistantAudio\(\)\) \{/, 'server should buffer binary assistant audio whenever ordering would be affected');
  assert.match(server, /let assistantAudioSuppressed = false;/, 'server should mute assistant audio after barge-in');
  assert.match(server, /if \(assistantAudioSuppressed\) return;/, 'server should drop late assistant audio after barge-in');
