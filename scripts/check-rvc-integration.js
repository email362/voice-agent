...
(async () => {
  ...
  assert.match(engine, /self\._check_cancelled\(cancelled\)[\s\S]*rvc = await self\._load_backend\(\)[\s\S]*self\._check_cancelled\(cancelled\)[\s\S]*await self\._conversion_lock\.acquire\(\)/, 'RVC conversions should not hold the conversion lock during backend cold-start');
